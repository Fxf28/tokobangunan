...
// --- LANGKAH 1: Tambahkan 'onNavigate' prop ---
// Ini adalah fungsi yang akan dipanggil setiap kali link di-klik
type NavContentProps = {
  onNavigate?: () => void;
};

const NavContent = ({ onNavigate }: NavContentProps) => {
  const { logout } = useAuth(); // --- BARU: Ambil fungsi logout ---
    ...
    <div className="flex h-full flex-col justify-between border-r border-gray-200 bg-white">
          ...
          <ul className="space-y-2">
            {navItems.map((item) => (
              <li key={item.name}>
                <Link
                  to={item.href}
                  onClick={onNavigate} // --- LANGKAH 2: Tambahkan onClick di sini ---
      ...
      <div className="border-t border-gray-200 p-4">
        <ul className="space-y-2">
          ...
          {/* --- TOMBOL LOGOUT DIUBAH --- */}
          <li>
            <a
              href="#"
              onClick={handleLogout} // --- BARU ---
              className="flex items-center gap-3 rounded-md px-4 py-3 text-sm font-medium text-gray-500 hover:bg-gray-50 hover:text-gray-900"
            >
              <LogOut size={18} />
              <span>Log Out</span>
...
const Sidebar = ({ isSidebarOpen, setSidebarOpen }: SidebarProps) => {
  // --- LANGKAH 3: Buat fungsi helper untuk menutup sidebar ---
  ...
  return (
    <>
      ...
      <div className="hidden md:flex md:w-64 md:flex-shrink-0">
        {/* Versi desktop tidak perlu 'onNavigate' */}
        <NavContent />
      </div>
          ...
          <div className="fixed inset-0 flex">
            ...
            >
              <Dialog.Panel className="relative flex w-64 flex-1 flex-col bg-white">
                ...
                {/* --- LANGKAH 4: Oper 'closeSidebar' ke 'onNavigate' ---
                  Sekarang, setiap <Link> di dalam NavContent versi mobile
                  akan memanggil closeSidebar() saat di-klik.
                */}
